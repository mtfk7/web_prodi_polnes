    ...
    <section className="py-20 bg-gradient-to-br from-green-700 to-emerald-800">
      ...
      <div className="container px-4">
        <div className="text-center mb-16">
          ...
          <p className="text-green-100 max-w-2xl mx-auto">Dengarkan pengalaman langsung dari lulusan kami yang telah sukses di industri teknologi</p>
        </div>
              ...
              {currentTestimonials.map((testimonial, index) => (
                <Card
                  ...
                  className={cn(
                    ...
                    isAnimating && (slideDirection === 'left' ? 'slide-in-from-right' : slideDirection === 'right' ? 'slide-in-from-left' : '')
                  )}
                >
                  <CardHeader>
                    ...
                    <div className="relative">
                      ...
                      <CardDescription className="text-green-100 text-base leading-relaxed pl-4 transition-colors duration-300">"{testimonial.testimonial}"</CardDescription>
                    </div>

                    <div className="mt-4 pt-4 border-t border-white/20">
                      <Badge variant="outline" className="text-green-200 border-green-300/30 transition-all duration-300 hover:bg-green-200 hover:text-green-800">
            ...
            {Array.from({ length: Math.ceil(testimonials.length / itemsPerView) }).map((_, index) => (
              <button
                ...
                className={`w-3 h-3 rounded-full transition-all duration-300 transform hover:scale-105 ${
                  index === Math.floor(currentIndex / itemsPerView) ? 'bg-white scale-105' : 'bg-white/30 hover:bg-white/50'
