...
import { Code2, Link as LucideLink, Palette, Smartphone, Sparkles, ChevronDown, ChevronUp } from "lucide-react"
import { programHighlights } from "@/data/ti/content"
import { Button } from "../ui/button"
import { useState } from "react"
import { ExploreProgramsContent } from "../ti/explore-programs-content-ti"
...
export function ProgramHighlights() {
  ...
  return (
    <section id="kurikulum" className="py-20 bg-emerald-50">
      <div className="container px-4">
        <div className="text-center mb-16">
          <Badge className="py-2 px-4 py-2 px-4 mb-4 bg-emerald-100 text-emerald-800">Sorotan Kurikulum</Badge>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {programHighlights.map((highlight, index) => {
            ...
            return (
              <Card
                key={index}
                className={`group hover:shadow-xl transition-all duration-300 border-0 bg-gradient-to-br ${highlight.gradient}`}
              >
                <CardHeader className="text-center pb-4">
                  <div
                    className={`mx-auto w-16 h-16 bg-gradient-to-br ${highlight.iconGradient} rounded-2xl flex items-center justify-center mb-4 group-hover:scale-110 transition-transform`}
                  >
        ...
        <div className="flex justify-center mt-12">
          <Button
            ...
            size="lg"
            className="bg-amber-400 text-black hover:bg-amber-300 font-semibold shadow-lg hover:shadow-xl transition-all duration-300"
          >
            {showExploreContent ? (
              <>
