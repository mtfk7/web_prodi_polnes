...
import { siteConfig } from "@/data/trk/content"
      ...
      <div className="w-full max-w-7xl mx-auto flex h-20 items-center justify-between px-4 md:px-6">
        ...
        <nav className="hidden md:flex flex-1 justify-center items-center gap-8">
          {navigation.map((item) => (
            <button
              ...
              className="relative text-cyan-100 hover:text-amber-400 font-medium transition-colors px-2 py-1 focus:outline-none group"
            >
              <span>{item.name}</span>
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button className="relative text-cyan-100 hover:text-amber-400 font-medium transition-colors px-2 py-1 focus:outline-none group flex items-center gap-1">
            ...
            <DropdownMenuContent
              align="center"
              className="w-64 bg-white/95 backdrop-blur border-cyan-200 shadow-xl"
            >
        ...
        <div className="flex items-center gap-2 min-w-max">
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="rounded-full bg-amber-400 text-cyan-900 font-bold shadow-lg px-6 py-2 hover:bg-amber-300 transition-all duration-200 text-lg hidden md:inline-flex items-center justify-center"
          >
            Daftar Sekarang
          </a>
          {/* Mobile menu button */}
          <Button
            variant="ghost"
            size="icon"
            className="md:hidden hover:text-white text-white hover:bg-[#212F4D]-800 focus:bg-[#212F4D]-800"
            ...
            <nav className="flex flex-col space-y-4">
              {navigation.map((item) => (
                <button
                  key={item.name}
                  onClick={() => handleNavClick(item.href)}
                  className="text-left text-cyan-200 hover:text-amber-400 transition-colors text-lg py-2"
                >
                  {item.name}
                </button>
              ))}
              ...
              <div className="space-y-2">
                <div className="text-cyan-200 font-medium text-lg py-2">Akses Cepat</div>
                {quickAccess.map((item) => (
                  <a
                    key={item.name}
                    href={item.href}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="block text-cyan-200 hover:text-amber-400 transition-colors text-base py-1 pl-4"
                  >
                    {item.name}
                  </a>
                ))}
              ...
              <Button
                ...
                variant="outline"
                size="lg"
                className="w-full mt-4 border-amber-400 text-amber-400 hover:bg-amber-400 hover:text-cyan-900 bg-transparent"
              >
                <a
                  href="https://pmb.polnes.ac.id"
                  target="_blank"
