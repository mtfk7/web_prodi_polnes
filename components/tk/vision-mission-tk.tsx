...
export function VisionMission() {
  return (
    <section id="visi-dan-misi" className="py-20 bg-gradient-to-br from-teal-700 to-teal-900">
      <div className="container px-4">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
            <Badge className="py-2 px-4 py-2 px-4 mb-4 bg-teal-500 text-white">Visi & Misi</Badge>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            <Card className="bg-white/10 backdrop-blur border-white/20 text-white">
              ...
              <CardContent>
                <p className="text-lg leading-relaxed text-teal-100">{visionMission.vision.content}</p>
              </CardContent>
            </Card>
                ...
                <ul className="space-y-3 text-teal-100">
                  {visionMission.mission.items.map((item, index) => (
                    <li key={index} className="flex items-start gap-2">
                      <Star className="h-5 w-5 text-teal-400 mt-0.5 flex-shrink-0" />
                      <span>{item}</span>
                    </li>
                  ))}
                </ul>
