...
export function GraduateResults() {
  ...
  return (
    <section className="py-20 bg-gradient-to-br from-teal-700 to-teal-600">
      <style jsx>{`
        .slide-in-from-right {
      ...
      <div className="container px-4">
        <div className="text-center mb-16">
          <Badge className="mb-4 bg-teal-200 text-teal-900 font-bold shadow">Testimoni Alumni</Badge>
          <h2 className="text-3xl md:text-4xl font-bold text-white mb-4 drop-shadow">Apa Kata Alumni Kami</h2>
          <p className="text-teal-100 max-w-2xl mx-auto drop-shadow">Dengarkan pengalaman langsung dari lulusan kami yang telah sukses di industri teknologi</p>
        </div>
        ...
        <div className="relative max-w-6xl mx-auto">
          ...
          <Button
            ...
            variant="outline"
            size="icon"
            className="absolute left-2 md:left-0 md:-translate-x-12 top-1/2 -translate-y-1/2 z-10 bg-teal-800/80 border-teal-300/40 text-white hover:bg-teal-700/80 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
          >
          ...
          <Button
            ...
            variant="outline"
            size="icon"
            className="absolute right-2 md:right-0 md:translate-x-12 top-1/2 -translate-y-1/2 z-10 bg-teal-800/80 border-teal-300/40 text-white hover:bg-teal-700/80 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 will-change-transform">
              {currentTestimonials.map((testimonial, index) => (
                <Card
                  ...
                  className={cn(
                    "h-full bg-teal-800/80 backdrop-blur border-teal-200/30 text-white hover:bg-teal-900/90 transition-all duration-500 ease-out transform hover:scale-103 hover:shadow-xl",
                    isAnimating &&
                      (slideDirection === "left"
                        ? "slide-in-from-right"
                ...
                >
                  <CardHeader>
                    <div className="flex items-start gap-4 mb-4">
                      <Avatar className="h-12 w-12 border-2 border-teal-200/40 transition-transform duration-300 hover:scale-105">
                        <AvatarFallback className="bg-teal-600 text-white font-semibold">
                          {testimonial.name.split(" ").map((n) => n[0]).join("")}
                        </AvatarFallback>
                      </Avatar>
                      <div className="flex-1">
                        <CardTitle className="text-lg font-semibold transition-colors duration-300 text-teal-100 drop-shadow">
                          {testimonial.name}
                        </CardTitle>
                        <CardDescription className="text-teal-200 text-sm transition-colors duration-300">
                        ...
                        <div className="flex items-center gap-1 mt-1">
                          {[...Array(testimonial.rating)].map((_, i) => (
                            <Star
                              key={i}
                              className="h-4 w-4 fill-yellow-400 text-yellow-300 transition-transform duration-300 hover:scale-100"
                            />
                          ))}
                        </div>
                      </div>
                    ...
                    <div className="relative">
                      <Quote className="h-6 w-6 text-teal-300 absolute -top-2 -left-2 opacity-60 transition-transform duration-300 hover:scale-100" />
                      <CardDescription className="text-teal-100 text-base leading-relaxed pl-4 transition-colors duration-300 drop-shadow">
                    ...
                    <div className="mt-4 pt-4 border-t border-teal-200/30">
                      <Badge
                        variant="outline"
                        className="text-teal-100 border-teal-300/30 transition-all duration-300 hover:bg-teal-200 hover:text-teal-900"
                      >
              ...
              <button
                ...
                className={`w-3 h-3 rounded-full transition-all duration-300 transform hover:scale-105 ${
                  index === Math.floor(currentIndex / itemsPerView)
                    ? "bg-teal-200 scale-105"
                    : "bg-teal-100/40 hover:bg-teal-200/80"
